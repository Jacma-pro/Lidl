...
| **Base légale** | **Art. 6.1.f — Intérêt légitime de l'employeur** : le suivi de performance est légitime dans le cadre d'une relation de travail, sous réserve que les données soient proportionnées, accessibles uniquement aux personnes concernées et non utilisées pour des décisions entièrement automatisées. |
| **Justification** | Le score synthétique (`global_score`) est calculé automatiquement mais n'aboutit à aucune décision automatisée (pas de licenciement automatique, pas de sanction sans intervention humaine). L'accès est strictement limité aux rôles Manager et Admin. |
