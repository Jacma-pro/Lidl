...
Toute l'infrastructure utilise un tunnel sécurisé HTTPS (protocole TLS 1.3) pour garantir que ces informations ne soient jamais interceptées lors de la transmission.

Pourquoi nous utilisons les tokens (JWT)
Pour vous permettre de naviguer facilement dans l’application développée avec le framework React sans avoir à vous reconnecter en permanence, Lidl Collect utilise une technologie appelée JWT (JSON Web Token).
...
La sécurité de votre compte commence dès la création de votre mot de passe. Une jauge de force est intégrée à l'interface pour s'assurer que le mot de passe respecte l'entropie recommandée par la CNIL.
...
L'accès aux ressources de Lidl Collect implique des responsabilités. Cette charte définit les règles que tout utilisateur s’engage à respecter :

Confidentialité : Les identifiants de connexion sont strictement personnels. Le partage de compte est formellement interdit.

Verrouillage : Tout utilisateur doit verrouiller sa session (Windows + L) dès qu'il quitte son poste de travail.

Sécurité MFA : L'usage de l'authentification forte est obligatoire pour le personnel interne. Tout dysfonctionnement doit être signalé au pôle réseau.

Logiciels : L'installation de logiciels non autorisés sur les outils de préparation est interdite pour éviter l'introduction de failles.
