...
> Destinataires : Willy (authentification, tokens), Patrice (infrastructure réseau)
...
| 🔴 Critique | Rotation non coordonnée du sel `HMAC_SECRET` | Une rotation sans remappage préalable rend orphelins tous les tokens pseudonymisés dans Order, AuditLog, Consent, Schedule, Performance — intégrité irréversiblement compromise (voir `politique-effacement.md`, section 2) |
...
| # | Menace | Criticité | Contre-mesure |
|---|---|:---:|---|
| S1.1 | **Attaque par force brute** sur `POST /api/auth/login` (credential stuffing, liste de mots de passe) | 🔴 | Rate limiting Nginx (`limit_req_zone`) + `ThrottlerGuard` NestJS + lockout temporaire après 5 tentatives échouées (→ Willy) |
...
| S1.3 | **JWT forgé avec `alg: none`** — token sans signature accepté par le serveur | 🔴 | Rejet explicite de `alg: none` dans la configuration JWT NestJS — `algorithms: ['HS256']` ou `['RS256']` au choix de Dorian selon l'architecture (matrice RBAC) — rejet de `alg: none` imposé dans tous les cas (→ Willy) |
| S1.4 | **Manipulation du champ `kid`** (JWT Key ID injection) — pointage vers une clé contrôlée par l'attaquant | 🟠 | Valider que `kid` correspond à une clé gérée en interne — ne jamais charger une clé dynamiquement depuis le payload (→ Willy) |
...
| T1.2 | **Interception et modification en transit** (Man-in-the-Middle sur connexion non chiffrée) | 🔴 | HTTPS obligatoire — TLS terminé en TB1 (Nginx) — aucune route HTTP en clair (→ Patrice) |
...
| I1.2 | **Email en clair dans les logs NestJS** — exfiltration possible si logs compromis | 🟠 | Interdiction formelle de logger les emails ou mots de passe — à confirmer avec Dorian (lacune documentée dans `politique-effacement.md`, section 1) |
...
| I1.4 | **Compromission du compte Brevo** — accès aux emails transactionnels (liens de réinitialisation, confirmations d'effacement Art. 17, codes de vérification d'identité hors-session) | 🟠 | DPA Brevo à signer avant production (action bloquante — `analyse-sous-traitants.md`) — rotation des clés API Brevo + surveillance des envois anormaux (→ Patrice) |

### D — Denial of Service (Déni de service)

| # | Menace | Criticité | Contre-mesure |
|---|---|:---:|---|
| D1.1 | **Flood sur `/api/auth/login`** — épuisement du pool de connexions PostgreSQL | 🟠 | Rate limiting Nginx + pool de connexions limité côté TypeORM (`max: 10`) (→ Patrice) |
| D1.2 | **Émission massive de refresh tokens** — saturation de la table de rotation | 🟡 | Révocation des tokens précédents à chaque renouvellement (rotation stricte) — un seul refresh token valide par session (→ Willy) |
...
| I2.2 | **Fuite inter-magasins** — un OPERATOR accède aux commandes d'un autre magasin | 🔴 | `StoreOwnershipGuard` : `req.user.storeId === order.storeId` — `403 Forbidden` sinon (→ Dorian) |
| I2.3 | **Exposition des données de commande dans le cache Nginx** — réponse API authentifiée servie à un autre utilisateur | 🔴 | `Cache-Control: private, no-store` sur toutes les routes API protégées (intercepteur NestJS global) + `proxy_no_cache 1` dans le bloc `/api/` Nginx (→ Dorian + David) — ⚠️ **Correction requise — non implémentée en l'état** (voir registre T13 et `analyse-sous-traitants.md`) |
| I2.4 | **IDOR sur SubstitutionProposal** — un CLIENT accepte ou consulte une substitution appartenant à la commande d'un autre client via `substitution_id` prévisible | 🟠 | Vérification `substitution.order.clientId === req.user.id` avant toute lecture ou action sur une substitution (→ Dorian) |
...
| S3.2 | **Vol du cookie de session ADMIN** — session hijacking | 🔴 | Refresh token ADMIN court (4 heures) + `httpOnly` + `SameSite=Strict` + `Secure` + MFA obligatoire à la connexion (→ Willy) |
| S3.3 | **Vol du cookie de session OPERATOR ou MANAGER** — session hijacking sur rôles à accès étendu (commandes magasin, stock, planning, performance) | 🔴 | MFA obligatoire OPERATOR et MANAGER (matrice RBAC) — refresh token 8 heures + `httpOnly` + `SameSite=Strict` + `Secure` (→ Willy) |
...
| DELETE | `/api/user/me` | CLIENT, OPERATOR, MANAGER, ADMIN | Flux d'effacement différent selon le rôle — pour OPERATOR/MANAGER : pseudonymisation de Schedule et Performance en sus ; notification Manager si OPERATOR ; droit d'opposition Art. 21 applicable (politique-effacement.md, section 8) |
| GET | `/api/user/export` | CLIENT+ | Endpoint d'exfiltration maximale : retourne email, nom, téléphone, adresse, commandes complètes, fidélité — I2.1 (IDOR si clientId en paramètre) — rate limiting requis — `AuditLog` : `EXPORT_REQUESTED` obligatoire (politique-effacement.md, section 7) |
...
| # | Scénario | Criticité | Contre-mesure |
|---|---|:---:|---|
| BL1 | **Annulation frauduleuse** — CLIENT annule la commande d'un autre via IDOR sur `DELETE /api/orders/:id` | 🔴 | Vérification systématique `order.clientId === req.user.id` avant toute mutation (→ Dorian) |
| BL2 | **Auto-validation de retrait** — CLIENT passe son statut à `PICKED_UP` via `PATCH /api/orders/:id/status` | 🟠 | Transition `PICKED_UP` autorisée uniquement pour OPERATOR, MANAGER, ADMIN — guard `RolesGuard` sur ce statut spécifiquement (→ Dorian) |
| BL3 | **Épuisement de stock par commandes fantômes** — CLIENT crée de nombreuses commandes puis les annule pour bloquer la disponibilité | 🟠 | Limite de commandes actives par compte (`max_active_orders` : 3) + rate limiting sur `POST /api/orders` (→ Dorian + Patrice) |
| BL4 | **Manipulation du montant final** — CLIENT tente de soumettre un prix modifié dans le corps de la requête de commande | 🔴 | Prix recalculé côté serveur depuis la table `Product` — le corps de la requête n'accepte que l'identifiant du produit et la quantité (→ Dorian) |

### Canal de notification email et SMS (Brevo)

| # | Menace | Criticité | Contre-mesure |
|---|---|:---:|---|
| N1 | **Compromission du compte Brevo** — un attaquant accède au dashboard Brevo et lit les emails transactionnels (liens Art. 17 hors-session, codes OTP) | 🟠 | Clés API Brevo stockées en variable d'environnement, jamais en dur — rotation régulière — alertes sur volume anormal d'envois (→ Patrice) |
| N2 | **Énumération de numéros de téléphone** via les erreurs de l'API SMS | 🟡 | Réponse générique en cas d'erreur d'envoi SMS — ne pas distinguer "numéro invalide" de "envoi échoué" dans la réponse API (→ Dorian) |
| N3 | **Injection de contenu dans les templates de notification** — contenu non sanitisé inséré dans le corps d'un email ou SMS | 🟠 | Sanitisation de toutes les variables insérées dans les templates Brevo — aucun champ libre de l'utilisateur directement dans le template (→ Dorian) |
| N4 | **Phishing via fausse notification Lidl Collect** — email spoofé imitant une notification de commande | 🟡 | Configuration SPF, DKIM, DMARC sur le domaine expéditeur — obligatoire avant mise en production (→ Patrice via Brevo) |

### QR code de retrait et validation physique

| # | Menace | Criticité | Contre-mesure |
|---|---|:---:|---|
| Q1 | **QR code prévisible ou devinable** — token de collecte généré côté client ou avec entropie insuffisante | 🔴 | Token de collecte généré côté serveur avec CSPRNG (module `crypto` Node.js) — UUID v4 ou équivalent — jamais calculé côté client (→ Dorian) |
| Q2 | **Réutilisation du QR code** — token valide après retrait effectué | 🟠 | Token invalidé à la première validation OPERATOR (`PICKED_UP`) — statut `USED` en base, toute tentative de validation ultérieure retourne `409 Conflict` (→ Dorian) |
| Q3 | **Forgé à partir du QR code d'une autre commande** — devinette par incrémentation d'un identifiant séquentiel | 🔴 | Token opaque (non basé sur l'`orderId` séquentiel) — lien token ↔ commande uniquement en base, jamais dans le token lui-même (→ Dorian + Sabry) |
...
| # | Menace | Criticité | Contre-mesure |
|---|---|:---:|---|
| EE1 | **CLIENT accède aux scores de performance d'un préparateur** via IDOR sur `GET /api/performance/:preparerId` | 🟠 | `RolesGuard` : route accessible Manager et Admin uniquement — OPERATOR n'accède qu'à ses propres scores via un endpoint séparé `/api/performance/me` (→ Dorian) |
| EE2 | **OPERATOR accède aux données de performance d'un autre préparateur** du même magasin | 🟠 | Vérification `performance.preparerId === req.user.id` pour le rôle OPERATOR — les scores des autres membres de l'équipe sont réservés au Manager (→ Dorian) |

---

## 7. Vecteurs d'escalade de privilèges — Synthèse

| # | Vecteur | Criticité | État | Contre-mesure |
|---|---|:---:|---|---|
| V1 | **IDOR** — accès à une ressource appartenant à un autre utilisateur via identifiant prévisible | 🔴 | À implémenter | `order.clientId === req.user.id` sur tout endpoint retournant une ressource par identifiant |
| V2 | **`alg: none` JWT** — token forgé sans signature | 🔴 | À implémenter | `algorithms: ['HS256']` ou `['RS256']` — choix architectural non encore arrêté (matrice RBAC) — rejet de `alg: none` imposé dans tous les cas (→ Willy) |
| V3 | **JWT `kid` injection** — chargement d'une clé externe contrôlée par l'attaquant | 🟠 | À implémenter | Validation que `kid` correspond à une clé connue et statique — aucun chargement dynamique (→ Willy) |
| V4 | **Endpoint admin non protégé** — route `/api/admin/*` sans guard actif | 🔴 | À implémenter | Guard global + scénario de test : CLIENT → `GET /api/admin/users` → `403` obligatoire (→ Dorian) |
| V5 | **Violation storeId** — OPERATOR accédant aux données d'un autre magasin | 🔴 | À implémenter | `StoreOwnershipGuard` : `user.storeId === resource.storeId` — appliqué sur toutes les routes OPERATOR (→ Dorian) |
| V6 | **Mass assignment** — champ `role` ou `storeId` modifiable via `PATCH` | 🔴 | À implémenter | `ValidationPipe` avec `whitelist: true` — champs `role` et `storeId` absents de tous les DTOs CLIENT (→ Dorian) |
| V7 | **Race condition** — double-submit de commande → stock négatif | 🟠 | À implémenter | Transaction PostgreSQL avec `SELECT FOR UPDATE` sur le stock (→ Dorian) |
| V8 | **Rotation non coordonnée du sel HMAC_SECRET** — tokens pseudonymisés orphelins dans toutes les tables sous obligation légale | 🔴 | Risque opérationnel | Remappage complet des tokens avant toute rotation — coordination obligatoire BDD + Cyber (politique-effacement.md, section 2) |
...
| Risque | Flux | Responsable |
|---|---|---|
| Force brute sur l'authentification | Flux 1 | Willy |
| `alg: none` JWT | Flux 1 | Willy |
| Vol de refresh token (cookie mal configuré) | Flux 1 | Willy |
| Mass assignment `role` | Flux 1 + 3 | Dorian |
| IDOR sur les commandes | Flux 2 | Dorian |
| Fuite inter-magasins (storeId) | Flux 2 + 3 | Dorian |
| Cache Nginx sur routes API authentifiées | Flux 2 + 3 | David + Dorian |
| Endpoint admin sans guard | Flux 3 | Dorian |
| Elevation via storeId injecté dans le corps | Flux 3 | Dorian |

### Risques élevés 🟠 — Traitement avant v1.0

| Risque | Flux | Responsable |
|---|---|---|
| Messages d'erreur différenciés (énumération comptes) | Flux 1 | Dorian |
| Injection SQL via paramètre non validé | Flux 2 | Dorian |
| Race condition stock négatif | Flux 2 | Dorian |
| JWT `kid` injection | Flux 1 | Willy |
| Vol de session ADMIN | Flux 3 | Willy |
| Vol de session OPERATOR / MANAGER (MFA obligatoire) | Flux 3 | Willy |
| Désactivation massive de comptes (ADMIN compromis) | Flux 3 | Willy + Patrice |
| Compromission compte Brevo (accès flux Art. 17 hors-session) | Flux 1 | Patrice |
| Rotation non coordonnée du sel HMAC_SECRET | Transversal | BDD + Cyber |

### Risques modérés 🟡 — Traitement en v1.x

| Risque | Flux | Responsable |
|---|---|---|
| Logs NestJS incluant des emails en clair | Flux 1 | Dorian |
| Flood sur les créneaux de retrait | Flux 2 | Patrice |
| Rotation des refresh tokens non stricte | Flux 1 | Willy |
| IDOR sur SubstitutionProposal | Flux 2 | Dorian |
| Rate limiting absent sur `GET /api/user/export` | Flux 2 | Dorian + Patrice |

---

## 9. Distribution des actions
...
- MFA obligatoire OPERATOR, MANAGER, ADMIN (les trois rôles — matrice RBAC)
...
- Rotation des clés API Brevo + surveillance des envois anormaux (menace I1.4)
...
- Deux blocs Nginx distincts : `/` (cache activé pour les assets) et `/api/` (`proxy_no_cache 1`)
- Voir `Lidl-Perso/conseils-equipe.md` — section « Cache Nginx »

---

← [matrice-rbac.md](matrice-rbac.md) | ← [registre-traitement.md](registre-traitement.md) | ← [politique-effacement.md](politique-effacement.md)
