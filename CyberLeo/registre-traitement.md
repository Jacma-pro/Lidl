...
| **Mesures de sécurité** | Mot de passe haché Bcrypt. Email vérifié par double opt-in (`is_verified`). Chiffrement au repos AES-256 (Supabase). Transport TLS. |
| **Lacunes documentées** | Algorithme Bcrypt confirmé par Willy — à valider par Dorian côté DTOs (mot de passe jamais exposé dans les réponses API). |
...
| **Mesures de sécurité** | MFA obligatoire pour OPERATOR, MANAGER, ADMIN (géré par Willy). Rejet explicite de `alg: none` dans NestJS. Payload JWT sans donnée personnelle. |
| **Lacunes documentées** | Stockage du refresh token : httpOnly cookie ou autre mécanisme — **non confirmé par Willy** (bloquant #4). Nginx logge-t-il les IPs ? durée de rétention — **non confirmé par David/Patrice** (bloquant #2). |
...
| **Note paiement** | MVP : `IN_STORE` ou `SIMULATED_ONLINE` uniquement — aucune donnée bancaire réelle ne transite. |
...
| **Durée de conservation** | **1 an** (Art. L34-1 — confirmé par David). Purge automatique recommandée : cron job mensuel ou TTL en base. |
| **Acteurs ayant accès** | ADMIN uniquement |
| **Transfert hors UE** | Non |
| **Mesures de sécurité** | `actor_id` pseudonymisé (HMAC-SHA256) à la suppression du compte concerné. Jamais de mot de passe ni de donnée de carte en clair dans les logs. |
...
| **Acteurs ayant accès** | OPERATOR (ses propres données) / MANAGER (données préparateurs de son magasin) / ADMIN (toutes) |
| **Transfert hors UE** | Non |
...
| **Acteurs ayant accès** | MANAGER (ses propres données) / ADMIN (toutes) |
| **Transfert hors UE** | Non |
...
| **Droit d'opposition** | Un préparateur peut s'opposer à ce traitement (Art. 21 RGPD). Procédure : gel de la table `Performance` pour son `preparer_id` + notification au Manager. Voir politique-effacement.md section 8. |
...
| **Transfert hors UE** | Non |
| **Risque identifié** | Donnée RH sensible — voir `dpia-analyse.md` (critère #1 — évaluation / scoring). Aucune décision automatique à effet juridique → DPIA non requise mais documentation recommandée. |
...
| **Durée de conservation** | **1 an** — Art. L34-1 (obligation de conservation des données de connexion). Confirmé par David dans ses notes d'infrastructure (source : Légifrance + CNIL). |
| **Acteurs ayant accès** | Équipe infrastructure (ADMIN système) |
| **Transfert hors UE** | Non — hébergement local / Docker |
| **Point de départ** | Notes David (`infradavid/mesnotes.md`) : cache serveur Nginx TTL 5-10 min, couvrant **à la fois les assets statiques et les routes API authentifiées**. Un seul cache pour tout. |
| **Analyse** | Nginx cache par URI par défaut. Sans clé de cache personnalisée par utilisateur, la réponse de `/api/profile` de l'utilisateur A peut être servie à l'utilisateur B pendant 5-10 min. Même avec clé JWT : révocation aveugle post-déconnexion, données personnelles écrites sur disque dans `/var/cache/nginx/`, maintenance silencieuse à chaque nouvelle route. La configuration actuelle manque de précision — elle pense performance, pas données personnelles. |
| **Point de sortie** | Deux configurations distinctes requises : (1) `/` assets statiques → cache activé, TTL long. (2) `/api/` routes authentifiées → `proxy_no_cache 1` dans Nginx + `Cache-Control: private, no-store` dans NestJS via intercepteur global sur le Guard JWT. Voir [conseils-equipe.md](../../Lidl-Perso/conseils-equipe.md) — section "Cache Nginx". |
...
| Logs Nginx | — | **6 mois max** (rec. CNIL) | |
| JWT access token | 15 min (CLIENT/OPERATOR), 10 min (ADMIN) | — | — |
| JWT refresh token | 7j (CLIENT), 8h (OPERATOR/MGR), 4h (ADMIN) | — | — |

---

## Lacunes documentées — Informations non obtenues à ce jour

> Ces lacunes ne compromettent pas la conformité d'ensemble, mais devront être levées préalablement à toute mise en production.

| # | Information manquante | Responsable | Impact RGPD |
|---|---|---|---|
| L01 | ~~Service email transactionnel (provider + pays serveurs + DPA)~~ | ~~Patrice~~ | ✅ **Résolu** — Brevo retenu (EU, AWS Irlande), DPA à signer |
| L02 | ~~Nginx : logge-t-il les IPs ? Durée de rétention exacte ?~~ | ~~David / Patrice~~ | ✅ **Résolu** — 1 an (Art. L34-1, notes David) |
| L03 | Nginx cache routes API authentifiées + assets statiques (TTL 5-10 min) — `Cache-Control: private, no-store` à implémenter | Dorian (NestJS) | ⚠️ **Risque de fuite inter-sessions** si non implémenté avant prod |
| L04 | Refresh token : stockage exact (httpOnly cookie ?) | Willy | T02 — base légale et durée de conservation du token |
| L05 | Sauvegardes Supabase : fréquence exacte (quotidienne ?) | Sabry | T01 — délai réel avant suppression effective en backup |

---

← [politique-effacement.md](politique-effacement.md) | → [analyse-sous-traitants.md](analyse-sous-traitants.md) | → [dpia-analyse.md](dpia-analyse.md)
